...
@Component
public class LaporanPengeluaranPelangganPdfExporter {
    ...
    public void export(
            List<LaporanPembelianPerPelangganDTO> rows,
            LocalDate startDate,
            LocalDate endDate,
        ...
        try {
            ...
            Paragraph title = new Paragraph("Laporan Pembelian Per Supplier", titleFont);
            ...
            int index = 1;
            for (LaporanPembelianPerPelangganDTO row : rows) {
                table.addCell(createCell(String.valueOf(index), Element.ALIGN_CENTER));
                table.addCell(createCell(
                        row.getSupplierName() == null ? "" : row.getSupplierName(),
                        Element.ALIGN_LEFT
                ));
                table.addCell(createCell(formatRupiah(row.getTotalHargaPembelian()), Element.ALIGN_RIGHT));
                index++;
            }
